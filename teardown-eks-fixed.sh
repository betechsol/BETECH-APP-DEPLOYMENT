...
remove_application() {
    ...
    print_status "Waiting for ALB to be deleted (this may take a few minutes)..."
    sleep 30
    
    # Remove application deployments
    print_status "Removing application deployments..."
    kubectl delete deployment betechnet-frontend --ignore-not-found=true
    kubectl delete deployment betechnet-backend --ignore-not-found=true
    kubectl delete deployment betechnet-postgres --ignore-not-found=true
    
    # Remove services
    print_status "Removing services..."
    kubectl delete service betechnet-frontend --ignore-not-found=true
    kubectl delete service betechnet-backend --ignore-not-found=true
    kubectl delete service betechnet-postgres --ignore-not-found=true
    
    # Remove secrets
    print_status "Removing secrets..."
    kubectl delete secret betechnet-secrets --ignore-not-found=true
    
    print_status "Application components removed successfully!"
}
...
remove_storage() {
    ...
    kubectl get pvc postgres-pvc --ignore-not-found=true || print_warning "PVC not found"
    ...
    kubectl delete storageclass betech-storage-class --ignore-not-found=true
    
    print_status "Persistent storage removed successfully!"
}
...
remove_alb_controller() {
    ...
    if command -v helm &> /dev/null && helm list -n kube-system | grep aws-load-balancer-controller &> /dev/null; then
        print_status "Removing ALB controller via Helm..."
        helm uninstall aws-load-balancer-controller -n kube-system
    else
        print_status "Removing ALB controller via kubectl..."
        kubectl delete -f manifests/aws-load-balancer-controller.yaml --ignore-not-found=true
    fi
    ...
    eksctl delete iamserviceaccount \
        ...
        --region=$REGION || print_warning "IAM service account might not exist"
    
    print_status "AWS Load Balancer Controller removed successfully!"
}
...
remove_ebs_csi_driver() {
    ...
    eksctl delete iamserviceaccount \
        ...
        --region=$REGION || print_warning "EBS CSI IAM service account might not exist"
    
    print_status "EBS CSI Driver components removed successfully!"
}
...
cleanup_iam_roles() {
    ...
    aws iam delete-role-policy --role-name AmazonEKSLoadBalancerControllerRole-pn4ipago --policy-name ALBAdditionalPermissions 2>/dev/null || print_warning "ALB additional permissions policy not found"
    
    print_status "Additional IAM roles cleaned up!"
}
